...
function AddComment()
{
      if (isset($_SESSION["status"]) && $_SESSION["status"] == "active")
      {
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                  ...
                  $sql = "INSERT INTO comment (user_id, comment, time_stamp, thread_comment_id) VALUES (:user_id, :comment, :time_stamp, :thread_comment_id)";
                  $statement = $db->prepare($sql);
                  ...
                  if ($statement->execute()) {
                        $comment_id = GetCommentId($db, $_POST['comment'], $time_stamp);

                        if ($comment_id != null) {
                              echo '<div class="thread" id="comment_id_' . $comment_id . '">'
                                    ...
                                    . '<div class="authorandcomment">'
                                          ...
                                          . '<div class="comment">' . $_POST['comment'] . '</div>
                                      </div>
                                      <div class="right-content">
                                                <button type="button" class="reply-btn" 
                                                onclick="openReplyBox(' . $_POST['thread_comment_id'] . ', ' . $comment_id . ', ' . $_SESSION['user_id'] . ')">Reply</button>
                                      </div>
                                    </div><br>';
